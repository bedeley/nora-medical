...
export async function GET(
  ...
  if (format !== "csv") {
    ...
    const subtitle = `${customer.name || customer.email || customer.id} · ${
      customer.email || ""
    }`;
    ...
    return new Response(pdf, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="customer-${customer.id}-statement.pdf"`,
      },
    });
  }
  ...
  return new NextResponse(body, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="customer-${customer.id}-statement.csv"`,
    },
  });
}
