...
export default function AdminB2BTendersPage() {
  ...
  const { data, refetch } = useClientQuery<{ items: TenderSnapshot[] }>({
    queryKey: ["admin", "b2b-tenders"],
    queryFn: () => fetcher("/api/admin/b2b/tenders"),
  });
  ...
  const isAutoSupplyNoteFormat = (note: string) => {
    ...
    return /^\d+\s+available now,\s+\d+\s+in\s+\d+\s+days?$/.test(clean);
  };
  ...
  const saveTender = async () => {
    ...
    if (oosPolicyViolations.length > 0) {
      toast.error(
        `Cannot save: ${oosPolicyViolations.length} out-of-stock line(s) require lead time + split supply note (available now and balance).`,
      );
      return;
    }
      ...
      .filter(
        (row, index, arr) =>
          (
            ...
            Boolean(row.supplyNote) ||
            Boolean(row.bidDisposition)
          ) &&
  ...
  const uploadItems = async (file: File | null) => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/admin/b2b/tenders/upload-items", {
        method: "POST",
        body: fd,
      });
  ...
  const sendTenderEmail = async () => {
    const tender = history.find((row) => row.id === sendTenderId) || activeTender || history[0];
    if (!tender) {
      toast.error("Save a tender first.");
      return;
    }
    if (!emailTo.trim()) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/b2b/tenders/${tender.id}/email`, {
        ...
        body: JSON.stringify({
          ...
          message: "Please find attached our tender submission.",
      ...
      if (!res.ok || !body.snapshot) {
        ...
        toast.error(firstFieldError || body.error || "Failed to send tender email");
        return;
      }
      setActiveTender(body.snapshot);
      setSendTenderId("");
  ...
  const compareVersions = async () => {
    ...
    if (!res.ok) {
      toast.error(body.error || "Failed to compare versions.");
      return;
    }
    setDiffResult(body);
  };
  ...
  const deleteTemplate = async (id: string) => {
    const res = await fetch(`/api/admin/b2b/tender-templates?id=${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
    const body = await res.json().catch(() => ({} as { error?: string }));
    if (!res.ok) {
  ...
  const loadFromProcurementRequest = useCallback(async (requestIdArg?: string) => {
    ...
    if (!requestId) {
      toast.error("Select a procurement request first.");
      return;
    }
    const reqRow = procurementRequests.find((row) => row.id === requestId);
    if (!reqRow) {
      toast.error("Selected procurement request not found.");
      return;
    }
    ...
    try {
      ...
      const linesText = body.draft.lines
        .map((line: { raw?: string; itemRef?: string; quantity?: number }) => `${line.itemRef || line.raw || "Item"}: ${line.quantity || 1}`)
        .join("\n");
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      void loadFromProcurementRequest(requestId);
    }, 0);
  }, [procurementRequests, searchParams, loadFromProcurementRequest]);

  useEffect(() => {
    void refreshApprovalStatus(sendTenderId || "");
  }, [sendTenderId]);

  const openPdf = (id: string) => {
    window.open(`/api/admin/b2b/tenders/${id}/pdf`, "_blank");
  };
  ...
  const loadTenderForEdit = (row: TenderSnapshot) => {
    ...
    setLotTitle(row.lotTitle || "");
    setCurrency(row.currency || "GHS");
    ...
    setPreview({
      ...
      matchedCount: (row.lines || []).filter((line) => Boolean(line.matchedProductId)).length,
      unmatchedCount: (row.lines || []).filter((line) => !line.matchedProductId).length,
    ...
    // Rebuild preview from live catalog so stock/cost/match data are current while preserving saved manual values.
    void (async () => {
      try {
        const res = await fetch("/api/admin/b2b/tenders/preview", {
          method: "POST",
        ...
        for (const liveLine of body.preview.lines) {
          ...
          if (Number(savedLine.quantity) !== Number(liveLine.quantity)) {
            ov.quantity = String(savedLine.quantity);
          }
          if (Number(savedLine.unitPrice) !== Number(liveLine.unitPrice)) {
            ov.unitPrice = String(savedLine.unitPrice);
          }
        ...
        // Keep snapshot preview if live refresh fails.
      }
    })();
    toast.success(
      row.status === "DRAFT"
        ? "Draft loaded for full editing."
        : "Non-draft loaded as a new draft template. Save will create a new tender draft.",
    );
  };
  ...
  const updateTenderStatus = async (tenderId: string) => {
    ...
    if (row.status === status) {
      toast.info("Tender is already in that status.");
      return;
    }
    ...
    return preview.lines.map((line) => {
      ...
      return {
        ...
        matchedProductId:
          bidDisposition === "NO_BID"
            ? null
            : resolvedProduct?.id || line.matchedProductId,
        matchedProductName:
          bidDisposition === "NO_BID"
            ? null
            : resolvedProduct?.name || line.matchedProductName,
        matchedSku:
          bidDisposition === "NO_BID"
            ? null
            : resolvedProduct?.sku || line.matchedSku,
        availableStock:
          bidDisposition === "NO_BID"
            ? null
            : resolvedProduct
  ...
  const effectiveTotal = useMemo(
    () => {
      const subtotal = effectiveLines.reduce((sum, line) => sum + line.lineTotal, 0);
      const vat = subtotal * (Math.max(0, Number(vatRatePct || 0)) / 100);
      const discount = Math.max(0, Number(discountAmount || 0));
      const freight = Math.max(0, Number(freightAmount || 0));
      const handling = Math.max(0, Number(handlingAmount || 0));
      return Math.max(0, subtotal + vat + freight + handling - discount);
    },
    [discountAmount, effectiveLines, freightAmount, handlingAmount, vatRatePct],
  );
  ...
  const marginViolations = useMemo(
    () =>
      effectiveLines.filter((line) => {
        ...
        const margin = ((line.unitPrice - cost) / cost) * 100;
  ...
  const reviewLines = useMemo(() => {
    ...
    return effectiveLines.filter((line) => {
      ...
      if (isOutOfStock) {
        ...
        if (!(hasLeadTime && hasRequiredSplitNote)) return true;
        return true;
      }
  ...
  return (
    <section className="container mx-auto py-8 max-w-7xl space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Upload/paste clinic tender items, match inventory, and generate branded tender PDF.
          </p>
        </div>
        <Link href="/admin/b2b/procurement" className="text-xs underline">
          Back to Procurement
        </Link>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Buyer / Facility</label>
              <Input value={buyerName} onChange={(e) => setBuyerName(e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Buyer contact</label>
              <Input value={buyerContact} onChange={(e) => setBuyerContact(e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Buyer email</label>
              <Input value={buyerEmail} onChange={(e) => setBuyerEmail(e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Tender reference</label>
          ...
          {sourceMode === "procurement" ? (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="sm:col-span-2 lg:col-span-3">
                <label className="mb-1 block text-xs text-muted-foreground">Procurement Request</label>
                <select
                  className="h-10 w-full rounded border bg-background px-3 text-sm"
                ...
                >
                  ...
                  {procurementRequests.map((row) => (
                    <option key={row.id} value={row.id}>
                      {row.id} | {row.clinicName} | {row.status} | {formatDateGH(row.updatedAt)}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <Input
                type="file"
                accept=".txt,.csv,.pdf,.doc,.docx,image/png,image/jpeg,image/webp,text/plain,text/csv,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          ...
          <div className="rounded border bg-muted/20 p-3 space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Button className="w-full sm:w-auto" variant="outline" onClick={saveTemplate} disabled={busy}>
                Save Template
              </Button>
            </div>
            {templates.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {templates.slice(0, 8).map((tpl) => (
                  <Button key={`tpl-del-${tpl.id}`} type="button" size="sm" variant="ghost" onClick={() => deleteTemplate(tpl.id)}>
                    Delete {tpl.name}
                  </Button>
                ))}
              </div>
          ...
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">
              Item list (one per line or CSV `description,unit,quantity`)
            </label>
            <Textarea rows={9} value={itemsText} onChange={(e) => setItemsText(e.target.value)} />
          </div>
          <div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button className="w-full sm:w-auto" variant="outline" onClick={analyze} disabled={busy}>
              Analyze Items
            </Button>
          </div>
          ...
          {lastSavedTender ? (
            <div className="rounded border border-emerald-300 bg-emerald-50 p-3 text-sm">
              ...
              <div className="mt-1 text-xs text-emerald-900/90">
                {lastSavedTender.tenderNumber} | {lastSavedTender.status} | {lastSavedTender.currency}{" "}
                {lastSavedTender.total.toFixed(2)} | Updated {formatDateGH(lastSavedTender.updatedAt)}
              </div>
              <div className="mt-2 flex flex-wrap gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => openPdf(lastSavedTender.id)}
                >
                  Open PDF
                </Button>
                {lastSavedTender.status === "DRAFT" ? (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => loadTenderForEdit(lastSavedTender)}
                  >
                    Edit Draft
                  </Button>
                ) : null}
      ...
      {preview ? (
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="text-xs text-muted-foreground">
              Subtotal: {preview.currency} {effectiveSubtotal.toFixed(2)} | VAT: {preview.currency} {effectiveVat.toFixed(2)} | Freight: {preview.currency} {Number(freightAmount || 0).toFixed(2)} | Handling: {preview.currency} {Number(handlingAmount || 0).toFixed(2)} | Discount: {preview.currency} {Number(discountAmount || 0).toFixed(2)}
            </div>
            {marginViolations.length > 0 ? (
              <div className="rounded border border-red-300 bg-red-50 p-2 text-xs text-red-900">
                Margin lock violation: {marginViolations.length} line(s) are below {minMargin.toFixed(2)}% minimum margin. Increase unit price or change product before saving.
              </div>
            ) : null}
                  ...
                  {visiblePreviewLines.map((line) => (
                    <tr
                      ...
                      className={`border-b last:border-0 ${
                        (line.availableStock != null && Number(line.availableStock || 0) < line.quantity) || line.matchConfidence === "NONE"
                          ? "bg-red-50"
                    ...
                    >
                      ...
                      <td className="px-2 py-2 text-right">{line.marginPct != null ? Number(line.marginPct).toFixed(2) : "-"}</td>
                      <td className="px-2 py-2 text-right">
                        <Input
                          className="h-8 text-right"
                      ...
                      <td className="px-2 py-2 text-right">{line.lineTotal.toFixed(2)}</td>
            ...
            <p className="text-xs text-muted-foreground">
              Quantity and Unit Price inputs above are manual overrides used when saving the tender draft.
            </p>
            <p className="text-xs text-muted-foreground">
              For scanned PDFs/images, OCR must be enabled on server (`B2B_TENDER_OCR_ENABLE=1` and `OCR_SPACE_API_KEY`).
            </p>
            {reviewLines.length > 0 ? (
              <div className="rounded border border-amber-300 bg-amber-50 p-3 text-sm space-y-2">
                ...
                <div className="max-h-64 overflow-auto space-y-2 text-xs text-amber-900">
                  {reviewLines.map((line) => (
                    <div key={`review-${line.no}`} className="rounded border border-amber-200 bg-white/70 p-2">
                      <div>
                        Line {line.no}: {line.requestedDescription} [{line.matchConfidence}]
                      </div>
                      <div className="mt-1">
                        <select
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              <Button className="w-full sm:w-auto" onClick={saveTender} disabled={busy}>
                Save Tender Draft
              </Button>
              {activeTender ? (
                <Button className="w-full sm:w-auto" variant="secondary" onClick={() => openPdf(activeTender.id)}>
                  Download / Print PDF
                </Button>
              ) : null}
            </div>
        ...
        <CardContent className="space-y-3">
          {approvalStatus ? (
            <div className={`rounded border p-2 text-xs ${
              approvalStatus.canSend ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-amber-300 bg-amber-50 text-amber-900"
            }`}>
              <div>
          ...
          <div>
            ...
            >
              <option value="">Latest active/latest saved</option>
              {history.map((row) => (
                <option key={`send-${row.id}`} value={row.id}>
                  {row.tenderNumber} | {row.buyerName}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <Input value={emailTo} onChange={(e) => setEmailTo(e.target.value)} placeholder="e.g., procurement@clinic.com" />
          </div>
          <div>
          ...
          <div className="flex items-end gap-2">
            <Button variant="outline" onClick={approveForSend} disabled={busy || approvalBusy || !sendTenderId}>
              Approve for Send
            </Button>
            <Button onClick={sendTenderEmail} disabled={busy || (approvalStatus?.requireApproval && !approvalStatus?.canSend)}>
              Send Latest Tender
            </Button>
          </div>
          </div>
          <div className="rounded border bg-muted/20 p-3 space-y-2">
            <div className="text-xs font-medium text-muted-foreground">Version Comparison</div>
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              <select
                className="h-10 w-full rounded border bg-background px-3 text-sm"
                ...
                disabled={!sendTenderId}
              >
                <option value="">From version (default previous)</option>
              ...
              <select
                ...
                disabled={!sendTenderId}
              >
                <option value="">To version (default latest)</option>
                {comparableVersions.map((v) => (
              ...
              <Button className="w-full sm:w-auto" variant="outline" onClick={compareVersions} disabled={!sendTenderId}>
                Compare
              </Button>
            </div>
            {diffResult ? (
              <div className="text-xs space-y-1">
                <div>
                  Compared v{diffResult.from.versionNo} ({diffResult.from.status}) {"->"} v{diffResult.to.versionNo} ({diffResult.to.status})
                </div>
                <div>Total change: {diffResult.totalsDelta.total >= 0 ? "+" : ""}{diffResult.totalsDelta.total.toFixed(2)}</div>
                <div>Changed lines: {diffResult.lineChanges.length}</div>
              </div>
            ) : null}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium">Recent Tenders</CardTitle>
        </CardHeader>
        <CardContent>
          {history.length === 0 ? (
            ...
            <div className="space-y-2">
              {history.map((row) => (
                <div key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded border p-2 text-sm">
                  {(() => {
                    const reminder = reminderByTenderId.get(row.id);
                    const orderLink = orderLinkByTenderId.get(row.id);
                    return (
                  <div>
                    <div className="font-medium">{row.tenderNumber} - {row.buyerName}</div>
                    <div className="text-xs text-muted-foreground">
                      {row.status} | {row.currency} {row.total.toFixed(2)} | Updated {formatDateGH(row.updatedAt)}
                      {reminder ? ` | Expires in ${reminder.daysToExpiry} day(s)` : ""}
                      {orderLink ? ` | Order created ${orderLink.orderId}` : ""}
                    </div>
                  </div>
                    );
                  })()}
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      ...
                      {allowedTargets.map((target) => (
                        <option key={`${row.id}-status-${target}`} value={target}>
                          {target}
                        </option>
                      ))}
                    </select>
                    ...
                    {row.status === "WON" && !orderLinkByTenderId.get(row.id) ? (
                    ...
                    {row.status === "WON" && orderLinkByTenderId.get(row.id) ? (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          (window.location.href = `/admin/orders/${encodeURIComponent(
                            orderLinkByTenderId.get(row.id)?.orderId || "",
                          )}`)
                        }
                      >
                        Open Order
