...
const normalizeLotCode = (value?: string | null) =>
  (value || "")
    .trim()
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-")
...
function pickEarliestDate(values: Array<Date | null | undefined>) {
  const dates = values.filter((d): d is Date => d instanceof Date);
...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    for (const [key, list] of groups.entries()) {
      ...
      const receivedAt = pickEarliestDate(sorted.map((row) => row.receivedAt)) || keeper.receivedAt;
      ...
      const purchaseId = keeper.purchaseId || sorted.find((row) => row.purchaseId)?.purchaseId || null;
      const supplierId = keeper.supplierId || sorted.find((row) => row.supplierId)?.supplierId || null;
